...
getSymbols("WM",from="2010-01-01",to="2021-10-01")
...
WM%>%Ad()%>%chartSeries()
WM%>%chartSeries(TA='addBBands();addBBands(draw="p");addVo();addMACD()',subset='2020')
...
WM_log_returns<-WM%>%Ad()%>%dailyReturn(type='log')
...
WM_mean_log<-mean(WM_log_returns)
...
mean_log<-c(WM_mean_log,FB_mean_log,TSLA_mean_log,AAPL_mean_log,GOOGL_mean_log)
mean_log<-round(mean_log,4)

#standard deviation of log stock returns

WM_sd_log<-sd(WM_log_returns)
...
sd_log<-c(WM_sd_log,FB_sd_log,TSLA_sd_log,AAPL_sd_log,GOOGL_sd_Log)
sd_log<-round(sd_log,4)

#create data frame
#Data frame contains the 4 companies with each company's average log return and standard deviation.
graphic1<-data.frame(rbind(c("WM",WM_mean_log,WM_sd_log),c("FB",FB_mean_log,FB_sd_log),c("TSLA",TSLA_mean_log,TSLA_sd_log),c("AAPL",AAPL_mean_log,AAPL_sd_log),c("GOOGL",GOOGL_mean_log,GOOGL_sd_Log)),stringsAsFactors = FALSE)


graphic1<-data.frame(mean_log,sd_log)
rownames(graphic1)<-c("WM","FB","TSLA","AAPL","GOOGL")
...
#Checking the correlation of 4 stocks: tesla, facebook, google, Waste Management

data<-cbind(diff(log(Cl(WM))),diff(log(Cl(GOOGL))),diff(log(Cl(AAPL))),diff(log(Cl(FB))))
...
#generate random daily exponent increase rate using WM's mean and sd log returns
...
#most recent price
price[1]<-as.numeric(WM$WM.Adjusted[length(WM$WM.Adjusted),])
...
random_data%>%ggplot(aes(Day,Price))+geom_line()+labs(title="Waste Management (WM) price simulation for 4 years")+theme_bw()
...
mc_matrix[1,1]<-as.numeric(WM$WM.Adjusted[length(WM$WM.Adjusted),])

for(j in 1:ncol(mc_matrix)){
  mc_matrix[1,j]<-as.numeric(WM$WM.Adjusted[length(WM$WM.Adjusted),])
...
final_mat%>%gather("Simulation","Price",2:501)%>%ggplot(aes(x=Day,y=Price,Group=Simulation))+geom_line(alpha=0.2)+labs(title="Waste Management Stock (WM): 500 Monte Carlo Simulations for 4 Years")+theme_bw()

#Print Projected Stock Price to Screen based on the following Probabilities
probs<-c(0.005,0.025,0.25,0.5,0.75,0.975,0.995)

WM_dist<-WM_log_returns%>%quantile(probs=probs,na.rm=TRUE)
WM_mean<-mean(WM_log_returns,na.rm=TRUE)
WM_sd<-sd(WM_log_returns,na.rm=TRUE)

WM_mean%>%exp() # 1.001271
...
WM2 <- data.frame(Date = index(WM), coredata(WM) )
...
stocksgrouped <-join_all(list(AAPL2, WM2, FB2, GOOGL2, TSLA2), by='Date', type='left')
